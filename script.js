...
async function loginSelected(type){const codigo=type==="empresa"?document.getElementById("companyLoginCode").value.trim():document.getElementById("userLoginCode").value.trim();const email=type==="usuario"?(document.getElementById("userLoginEmail")?.value||"").trim().toLowerCase():"";if(type==="usuario"&&!email)return alert("Digite seu e-mail.");if(type==="usuario"&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return alert("Digite um e-mail válido.");if(!codigo)return alert("Digite seu código de acesso.");await loginByCode(codigo,type,email)}
async function loginByCode(codigo,expectedType,email=""){showLoader("Carregando seu painel...");const res=await api("loginClient",{codigo,email});hideLoader();if(!res.ok){alert("E-mail ou código de acesso inválido.");return}if(res.type!==expectedType){alert("E-mail ou código de acesso inválido.");return}session=res;localStorage.setItem("pegaleva_client",JSON.stringify(session));openPanel()}
