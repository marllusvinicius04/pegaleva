...
let codeTimer;function checkCodeLive(inputId,msgId){clearTimeout(codeTimer);codeTimer=setTimeout(async()=>{const codigo=document.getElementById(inputId).value.trim();if(!codigo){document.getElementById(msgId).innerText="";return}const res=await api("checkCode",{codigo});if(res.available){document.getElementById(msgId).innerText="Código disponível.";document.getElementById(msgId).style.color="#047857"}else{document.getElementById(msgId).innerText="Código de acesso inválido. Escolha outro código.";document.getElementById(msgId).style.color="#ef4444"}},350)}
...
async function validateNewUserCode(codigo){const res=await api("checkCode",{codigo});const msg=document.getElementById("userCodeMsg");if(!res||res.available!==true){if(msg){msg.innerText="Código de acesso inválido. Escolha outro código.";msg.style.color="#ef4444"}return false}if(msg){msg.innerText="Código disponível.";msg.style.color="#047857"}return true}
async function showUserTermsStep(){const data=getUserCreateData();if(!data.nome||!data.whatsapp||!data.email||!data.cpf||!data.codigo){alert("Preencha todos os dados do usuário para continuar.");return}const btn=document.getElementById("nextUserTermsBtn");if(btn)btn.disabled=true;const codigoDisponivel=await validateNewUserCode(data.codigo);if(btn)btn.disabled=false;if(!codigoDisponivel){alert("Código de acesso inválido. Escolha outro código.");return}pendingUserRegisterData=data;document.getElementById("userCreateFormStep").style.display="none";document.getElementById("userTermsStep").style.display="block"}
async function createUserAccount(){const data=pendingUserRegisterData||getUserCreateData();if(!data.nome||!data.whatsapp||!data.email||!data.cpf||!data.codigo){alert("Preencha todos os dados do usuário para continuar.");return}const btn=document.getElementById("createUserBtn");if(btn)btn.disabled=true;const codigoDisponivel=await validateNewUserCode(data.codigo);if(!codigoDisponivel){if(btn)btn.disabled=false;alert("Código de acesso inválido. Escolha outro código.");resetUserCreateSteps();return}showLoader("Criando conta de usuário...");const res=await api("registerUser",data);hideLoader();if(btn)btn.disabled=false;if(!res.ok){alert(res.message||res.error||"Não foi possível criar.");return}await loginByCode(data.codigo,"usuario")}
async function loginSelected(type){const codigo=type==="empresa"?document.getElementById("companyLoginCode").value.trim():document.getElementById("userLoginCode").value.trim();if(!codigo)return alert("Digite seu código de acesso.");await loginByCode(codigo,type)}
async function loginByCode(codigo,expectedType){showLoader("Carregando seu painel...");const res=await api("loginClient",{codigo});hideLoader();if(!res.ok){alert(res.error||"Código não encontrado.");return}if(res.type!==expectedType){alert(expectedType==="empresa"?"Esse código não pertence a uma empresa.":"Esse código não pertence a um usuário.");return}session=res;localStorage.setItem("pegaleva_client",JSON.stringify(session));openPanel()}
