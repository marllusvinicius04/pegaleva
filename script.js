...
function selectAccessType(type){document.getElementById("accessOptions").style.display="none";document.getElementById("backBtn").style.display="block";if(type==="usuario"){document.getElementById("accessTitle").innerText="Acesso do usuário";document.getElementById("accessSubtitle").innerText="Entre ou crie sua conta para solicitar uma entrega.";document.getElementById("userAccess").style.display="block";document.getElementById("companyAccess").style.display="none";showUserTab("login")}else{document.getElementById("accessTitle").innerText="Acesso da empresa";document.getElementById("accessSubtitle").innerText="Entre com o código da empresa já cadastrada.";document.getElementById("userAccess").style.display="none";document.getElementById("companyAccess").style.display="block"}}
...
(function(){
  ...
  if(typeof originalLogout==="function"){
    window.logout=function(){
      const btn=document.getElementById("companyQuickRequestBtn");
      if(btn)btn.style.display="none";
      return originalLogout.apply(this,arguments);
    };
  }

  window.updateCompanyQuickButton=function(){
    const btn=document.getElementById("companyQuickRequestBtn");
    if(btn)btn.style.display=session&&session.type==="empresa"?"flex":"none";
  };

  window.updateCompanyQuickBairros=function(){
    const city=(document.getElementById("companyQuickCity")||{}).value||"Uruçuí";
    const select=document.getElementById("companyQuickBairro");
    if(!select)return;
    const list=city==="Benedito Leite"?["Benedito Leite"]:BAIRROS_URUCUÍ;
    select.innerHTML='<option value="">Selecione</option>'+list.map(b=>`<option>${b}</option>`).join("");
  };

  window.openCompanyQuickRequest=function(){
    if(!session||session.type!=="empresa")return;
    updateCompanyQuickBairros();
    const price=document.getElementById("companyQuickPriceBox");
    if(price){price.style.display="none";price.innerHTML="";}
    document.getElementById("companyQuickRequestModal").classList.add("active");
  };

  window.closeCompanyQuickRequest=function(event){
    if(event&&event.target!==document.getElementById("companyQuickRequestModal"))return;
    const modal=document.getElementById("companyQuickRequestModal");
    if(modal)modal.classList.remove("active");
  };

  window.closeCompanyQuickConfirmation=function(event){
    if(event&&event.target!==document.getElementById("companyQuickConfirmModal"))return;
    const modal=document.getElementById("companyQuickConfirmModal");
    if(modal)modal.classList.remove("active");
  };

  window.prepareCompanyQuickConfirmation=async function(){
    if(!session||session.type!=="empresa")return;
    const city=(document.getElementById("companyQuickCity").value||"").trim();
    const bairro=(document.getElementById("companyQuickBairro").value||"").trim();
    const receiver=(document.getElementById("companyQuickReceiver").value||"").trim();
    const content=(document.getElementById("companyQuickContent").value||"").trim();
    const payment=(document.getElementById("companyQuickPayment").value||"").trim();
    if(!bairro)return alert("Informe o bairro de entrega.");
    if(!receiver)return alert("Informe o nome de quem vai receber.");
    if(!content)return alert("Informe o conteúdo da entrega.");
    if(!payment)return alert("Informe a forma de pagamento.");

    const p=session.profile||{};
    const bairroColeta=p.Bairro||p.bairro||"Centro";
    const coletaCidade=p.Cidade||p.cidade||"Uruçuí";
    const desconto=Number(p.EntregasComDescontoRestantes||0)>0?Number(p.DescontoPercentual||0):0;
    ...
    const price=await api("getPrice",{
      ...
      desconto,
      cupom:"",
      rotaRetorno:document.getElementById("companyQuickReturn").value,
      forcePriceFresh:true
    });
    hideLoader();
    if(!price.ok)return alert(price.error||"Não foi possível calcular o frete.");
    const box=document.getElementById("companyQuickPriceBox");
    if(box){
      box.style.display="block";
      box.innerHTML="<strong>Valor do frete: "+money(price.valor||0)+"</strong>";
    }
    document.getElementById("companyQuickConfirmModal").classList.add("active");
  };

  window.confirmCompanyQuickDelivery=async function(){
    if(!session||session.type!=="empresa")return;
    const p=session.profile||{};
    const city=(document.getElementById("companyQuickCity").value||"").trim();
    const bairro=(document.getElementById("companyQuickBairro").value||"").trim();
    const receiver=(document.getElementById("companyQuickReceiver").value||"").trim();
    const content=(document.getElementById("companyQuickContent").value||"").trim();
    const volumes=document.getElementById("companyQuickVolumes").value;
    const rota=document.getElementById("companyQuickReturn").value;
    const payment=document.getElementById("companyQuickPayment").value;
    const coletaCidade=p.Cidade||p.cidade||"Uruçuí";
    const bairroColeta=p.Bairro||p.bairro||"Centro";
    const enderecoColeta=p.Endereco||p.Rua||p.endereco||"Endereço cadastrado da empresa";
    const referenciaColeta=p.Referencia||p.referencia||"Empresa solicitante";

    closeCompanyQuickConfirmation();
    closeCompanyQuickRequest();
    showLoader("Buscando entregador...",true);
    const res=await api("createDelivery",{
      tipoCliente:"empresa",
      codigoCliente:p.CodigoAcesso,
      enderecoColeta:enderecoColeta,
      bairroColeta,
      coletaCidade,
      enderecoDestino:"LOCALIZAÇÃO EM TEMPO REAL SERÁ ENVIADA AO ENTREGADOR",
      referenciaColeta,
      referenciaDestino:"Solicitante enviará localização em tempo real após o aceite",
      bairroDestino:bairro,
      destinoCidade:city,
      nomeDestino:receiver,
      whatsappDestino:"",
      conteudo:content,
      volumes,
      pagamento:payment,
      ...
      cupom:"",
      rotaRetorno:rota,
      ofertaEntrega:"normal",
      localizacaoTempoReal:true,
      termoLocalizacaoAceito:true
    });
    if(!res.ok){
      hideLoader();
    ...
    currentSearchingId=res.delivery.ID;
    document.getElementById("companyQuickReceiver").value="";
    document.getElementById("companyQuickBairro").value="";
    document.getElementById("companyQuickContent").value="";
    document.getElementById("companyQuickPayment").value="";
    refreshPanel();
  };

  if(document.readyState==="loading"){
    document.addEventListener("DOMContentLoaded",updateCompanyQuickButton);
  }else{
    updateCompanyQuickButton();
  }
})();
